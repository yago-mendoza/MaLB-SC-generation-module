...
from src.coalitions.coordinator import CoordinationTeam
from src.coalitions.parser import ParsingTeam
...
def _filename_gen(extension: str) -> str:
...
def main():
    ...
    openai_logo_dir = Path("src/assets/openai-2.svg")
    sony_logo_dir = Path("src/assets/SONY.png")
    ...
    json_path = Path(__file__).parent / 'src' / 'assets' / 'sidebar_content.json'
    ...
    with HistoryTab:
        ...
        for label, obj, dt in get_manager().BACK_DATA:
            ...
            str_datetime = str(dt.strftime("%H:%M:%S"))
            dt = str(dt)
        ...
        json_path = Path(__file__).parent / 'output' / 'logs' / _filename_gen("json")
        print(json_path)
    ...
    with ParserTab:
        if st.session_state.get("selected_description"):  # Ensuring the key exists and has value
            ...
            # Saving "attributes" and "description" at @output folder ###########

            json_base_path = Path(__file__).parent / "output"

            attributes_path = json_base_path / "features" / _filename_gen("json")
            os.makedirs(attributes_path.parent, exist_ok=True)
            with attributes_path.open("w") as f:
                json.dump([a.dict() for a in attributes], f, indent=4)

            description = st.session_state["selected_description"] 

            description_path = json_base_path / "descriptions" / _filename_gen("json")
            os.makedirs(description_path.parent, exist_ok=True)
            with description_path.open("w") as f:
                json.dump(str(description), f, indent=4)
