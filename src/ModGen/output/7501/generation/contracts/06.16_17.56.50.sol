...
contract VIPAuction {
    ...
    function placeBid(uint _bidAmount) external payable {
        ...
        require((_bidAmount * 100) / highestBid >= 105, "Bid amount must be at least 5% higher than current highest bid");
        require(addressBidCount[msg.sender] < 3, "Maximum of 3 bids per address reached");
        
        if (block.timestamp >= auctionEndTime - 5 minutes) {
            auctionEndTime += 10 minutes; // Automatically extend auction by 10 minutes if new bid within last 5 minutes
        }
